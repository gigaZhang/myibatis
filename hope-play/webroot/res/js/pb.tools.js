 ...
 * 需要jquery.blockUI.js
 */
PB.Tools.loadingFunc = function(domid)
{
	var opt = {
            message : '<img src="' +PB.ContextPath + '/images/loading.gif" width="100%" height="20" />',
            css : {
                width : '160px'
            },
            cursor : 'wait',
...
function defaultDatePicker(startTm, endTm, monthlimits){
	...
	function format(year, month, date){
		return year + '-' + ((month < 10 ? ('0' + (month)) : month)) + '-' + ((date < 10 ? ('0' + date) : date)) + ' 00:00';
	}
}
...
PB.Tools.datePickerGroup = function(startTm, endTm, monthlimits){
	...
	document.getElementById(endTm).onfocus = function(){
		WdatePicker({
			...
			maxDate:'#F{$dp.$D(\'' + startTm + '\',{M:+' + monthlimits + '})||\'%y-%M-%d %H:%m\';}'
		});
	};
